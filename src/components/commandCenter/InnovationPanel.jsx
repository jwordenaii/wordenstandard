...
export default function InnovationPanel() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      await api.createInnovation({
        ...form,
        cost_to_test: form.cost_to_test !== '' ? Number(form.cost_to_test) : null,
      })
      setForm(BLANK)
      setShowAdd(false)
      load()
  ...
  const filtered = innovations.filter(
    (i) => (!filterCat || i.category === filterCat) && (!filterResult || i.result === filterResult)
  )
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap items-center justify-between">
        <div className="flex gap-2">
          {[
            { id: 'log', label: `🧪 Lab Log (${innovations.length})` },
            { id: 'adopted', label: `✅ Adopted (${adopted?.total_adopted ?? 0})` },
          ].map((t) => (
            <button
              key={t.id}
              type="button"
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                required
                placeholder="e.g. Drone Survey, 3D Paver Control"
            ...
            <div>
              ...
              <input
                type="date"
                value={form.date_tested}
                onChange={(e) => set('date_tested', e.target.value)}
                className="input text-sm w-full"
              />
            </div>
            <div>
              ...
              <input
                ...
                min="0"
                step="any"
                value={form.cost_to_test}
                onChange={(e) => set('cost_to_test', e.target.value)}
                className="input text-sm w-full"
              />
            </div>
            <div>
            ...
            {filtered.length === 0 ? (
              <p className="text-center text-brand-navy/30 py-8 text-sm">
                {innovations.length === 0
                  ? 'No trials logged yet. Start your innovation lab.'
                  ...
                  <tbody>
                    {filtered.map((i) => (
                      <tr key={i.id} className="border-b border-brand-navy/5">
                        ...
                        <td className="py-2.5 pr-3">
                          <span
                            className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${RESULT_STYLES[i.result] || 'bg-gray-100 text-gray-600'}`}
                          >
                            {i.result}
                          </span>
                        </td>
        ...
        <div className="space-y-4">
                  ...
                  <div key={cat} className="bg-green-50 rounded-xl px-4 py-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {methods.map((m) => (
                        <span
                          key={m}
                          className="bg-green-200 text-green-800 text-xs font-semibold px-2 py-0.5 rounded-full"
                        >
                          {m}
                        </span>
                      ))}
          ...
          {adopted.methods?.length === 0 && (
            <div className="text-center py-12 text-brand-navy/30 text-sm">
              No methods adopted yet. Mark trials as &quot;adopted&quot; to build your competitive
              intelligence asset.
            </div>
          )}
        </div>
