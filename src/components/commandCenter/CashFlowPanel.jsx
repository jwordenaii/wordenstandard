...
function WeekBar({ week }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <div
        className={`w-20 text-right font-semibold ${week.net >= 0 ? 'text-green-700' : 'text-red-600'}`}
      >
        {week.net >= 0 ? '+' : ''}
        {week.net.toLocaleString('en-US', {
          style: 'currency',
          currency: 'USD',
      ...
      <div
        className={`w-20 text-right text-xs ${week.cumulative >= 0 ? 'text-brand-navy/50' : 'text-red-500 font-bold'}`}
      >
        {week.cumulative.toLocaleString('en-US', {
          style: 'currency',
          currency: 'USD',
          minimumFractionDigits: 0,
        })}
        ...
        <div className="card p-4 bg-red-50 border-red-300 text-red-700 flex items-center gap-3">
          ...
          <div>
            <div className="font-bold">
              Cash Alert: Projected balance dips below ${Number(threshold).toLocaleString()}
            </div>
            <div className="text-sm">
              Min projected:{' '}
              {forecast.min_projected_balance.toLocaleString('en-US', {
                style: 'currency',
                currency: 'USD',
                minimumFractionDigits: 0,
              })}
      ...
      {tab === 'forecast' && forecast && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="card p-4 text-center">
              <div
                className={`font-display font-black text-2xl ${forecast.min_projected_balance >= 0 ? 'text-brand-amber' : 'text-red-600'}`}
              >
                {forecast.min_projected_balance.toLocaleString('en-US', {
                  style: 'currency',
                  currency: 'USD',
          ...
          <div className="card p-6">
            {entries.length === 0 ? (
              <p className="text-center text-brand-navy/30 py-8 text-sm">
                No entries yet. Add income and expenses to build the forecast.
              </p>
            ) : (
                    ...
                    {entries.map((e) => (
                      <tr key={e.id} className="border-b border-brand-navy/5">
                        <td className="py-2.5 pr-3 text-xs text-brand-navy/60">
                          {new Date(e.expected_date).toLocaleDateString()}
                        </td>
                        <td className="py-2.5 pr-3">
                          <span
                            className={`text-xs font-bold px-2 py-0.5 rounded-full ${e.entry_type === 'income' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                          >
                            {e.entry_type}
                          </span>
                        </td>
                        <td
                          className={`py-2.5 pr-3 font-semibold ${e.entry_type === 'income' ? 'text-green-700' : 'text-red-600'}`}
                        >
                          {e.entry_type === 'income' ? '+' : '-'}${e.amount.toLocaleString()}
                        </td>
                        <td className="py-2.5 pr-3 text-brand-navy/60 text-xs hidden sm:table-cell">
                          {e.category || '—'}
                        </td>
                        <td className="py-2.5 text-brand-navy/70 text-xs">
                          {e.description || '—'}
                        </td>
                        <td className="py-2.5">
                          <button
      ...
      {tab === 'settings' && (
        <div className="card p-6 max-w-sm">
          ...
          <p className="text-sm text-brand-navy/60 mb-4">
            Trigger an alert when the 13-week projected cumulative cash balance drops below this
            amount.
          </p>
