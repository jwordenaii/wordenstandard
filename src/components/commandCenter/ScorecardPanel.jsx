    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap items-center justify-between">
        <div className="flex gap-2">
          {[
            { id: 'portfolio', label: '📊 Portfolio Trends' },
            { id: 'projects', label: `📋 Projects (${metrics.length})` },
          ].map((t) => (
            <button
              key={t.id}
              type="button"
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              [
                'Cost Accuracy',
                avgs.cost_accuracy_pct != null ? `${avgs.cost_accuracy_pct}%` : '—',
                'text-blue-600',
              ],
              [
                'On-Time Rate',
                avgs.schedule_adherence_pct != null ? `${avgs.schedule_adherence_pct}%` : '—',
                'text-green-600',
              ],
              [
                'Avg Client NPS',
                avgs.avg_client_nps != null ? `${avgs.avg_client_nps}/10` : '—',
                'text-purple-600',
              ],
          ...
          <div className="card p-6 space-y-4">
            ...
            <MetricBar
              ...
              value={avgs.schedule_adherence_pct}
              color="bg-green-500"
            />
            <MetricBar
              label="Punch List Closure"
              value={avgs.punch_closure_pct}
        ...
        <div className="space-y-4">
            ...
            <div className="card p-5 bg-amber-50 border-amber-200">
              ...
              <div className="flex gap-3 mt-3">
                <button
                  ...
                  className="text-xs px-3 py-1 bg-white rounded-full border border-brand-navy/20 text-brand-navy hover:bg-gray-50"
                >
                  📋 Copy
                </button>
                <button
                  onClick={() => setCaseStudy(null)}
                  className="text-xs text-brand-navy/40 underline"
                >
                  Dismiss
                </button>
              </div>
          ...
          {metrics.length === 0 ? (
            <div className="text-center py-16 text-brand-navy/30 text-sm">
              No scorecards yet. Add your first completed project.
            </div>
          ) : (
            metrics.map((m) => (
              <div key={m.id} className="card p-5 space-y-3">
                ...
                <MetricBar
                  label="Schedule Adherence"
                  value={m.schedule_adherence_pct}
                  color="bg-green-500"
                />
                <MetricBar
                  label="Punch List Closure"
                  value={m.punch_closure_pct}
                  color="bg-brand-amber"
                />
                <NPSBar value={m.client_nps} />
                {m.case_study_published && m.case_study_text && (
                  <div className="text-xs text-brand-navy/50 bg-gray-50 rounded-lg px-3 py-2 line-clamp-2">
                    {m.case_study_text}
                  </div>
                )}
              </div>
