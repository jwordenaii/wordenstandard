...
function OutcomeTag({ outcome }) {
  ...
  return (
    <span
      className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${styles[outcome] || styles['no-decision']}`}
    >
      {outcome}
    </span>
  )
...
export default function BidIntelPanel() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = { ...form }
      ;['lead_id', 'proposal_amount_low', 'proposal_amount_high', 'competitor_price'].forEach(
        (k) => {
      ...
      <div className="flex gap-2 flex-wrap items-center justify-between">
        <div className="flex gap-2">
          {[
            ...
            { id: 'history', label: `📋 History (${outcomes.length})` },
          ].map((t) => (
            <button
              key={t.id}
              type="button"
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Richmond VA, Raleigh NC…"
                className="input text-sm w-full"
              />
            </div>
            <div>
              ...
              <input
                ...
                min="0"
                step="any"
                value={form.proposal_amount_low}
                onChange={(e) => set('proposal_amount_low', e.target.value)}
                className="input text-sm w-full"
              />
            </div>
            <div>
              ...
              <input
                ...
                min="0"
                step="any"
                value={form.proposal_amount_high}
                onChange={(e) => set('proposal_amount_high', e.target.value)}
                className="input text-sm w-full"
              />
            </div>
            <div>
                  ...
                  <div key={s.service}>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="font-bold text-brand-navy">
                        {s.win_rate_pct}% ({s.won}/{s.total})
                      </span>
                    </div>
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-green-500 rounded-full"
                        style={{ width: `${s.win_rate_pct}%` }}
                      />
                    </div>
                  </div>
                ))}
          ...
          <div className="card p-6">
            ...
            {analysis ? (
              ...
              <p className="text-sm text-brand-navy/40">
                Click &quot;Analyze&quot; to get AI-powered win/loss insights based on your bid
                history.
              </p>
            )}
          </div>
        ...
        <div className="card p-6">
          {outcomes.length === 0 ? (
            <p className="text-center text-brand-navy/30 py-8 text-sm">
              No bid outcomes recorded yet. Start tracking to unlock AI insights.
            </p>
          ) : (
                ...
                <tbody>
                  {outcomes.map((o) => (
                    <tr key={o.id} className="border-b border-brand-navy/5">
                      <td className="py-2.5 pr-3 text-xs text-brand-navy/50">
                        {new Date(o.outcome_recorded_at).toLocaleDateString()}
                      </td>
                      ...
                      <td className="py-2.5 text-brand-navy/60 text-xs hidden md:table-cell">
                        {o.competitor_name || '—'}
                        {o.competitor_price ? ` @ $${o.competitor_price.toLocaleString()}` : ''}
                      </td>
                      <td className="py-2.5">
                        <button
