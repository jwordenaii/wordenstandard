...
function KPITile({ kpi }) {
  ...
  const displayValue = kpi.value != null
    ? kpi.unit === '$'
      ? kpi.value.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 })
      : `${kpi.value}${kpi.unit || ''}`
    : 'No data'
  const targetDisplay = kpi.unit === '$'
    ? kpi.target.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 })
    : `${kpi.target}${kpi.unit || ''}`

  return (
    <div className={`card p-5 border ${style.bg} space-y-2`}>
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-brand-navy/60 uppercase tracking-wide">{kpi.label}</span>
      ...
      {kpi.total_bids != null && <div className="text-xs text-brand-navy/40">{kpi.total_won}/{kpi.total_bids} bids won</div>}
      {kpi.total_projects != null && <div className="text-xs text-brand-navy/40">{kpi.on_time_projects ?? kpi.total_projects} / {kpi.total_projects} projects</div>}
      {kpi.recordable_incidents != null && <div className="text-xs text-brand-navy/40">{kpi.recordable_incidents} recordable incident{kpi.recordable_incidents !== 1 ? 's' : ''}</div>}
      {kpi.total_members != null && <div className="text-xs text-brand-navy/40">{kpi.total_members} tracked members</div>}
    </div>
  )
}
...
function MonthBar({ month, count, max }) {
  ...
  return (
    <div className="flex items-end gap-1 flex-col text-center" style={{ width: '7%' }}>
      <div className="w-full bg-brand-amber rounded-t" style={{ height: `${Math.max(pct, 3)}px`, minHeight: 3 }} />
...
export default function KPIWallPanel() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-32 text-brand-navy/40 text-sm gap-3">
      <span className="w-5 h-5 border-2 border-brand-amber border-t-transparent rounded-full animate-spin" /> Loading KPI Wall…
    </div>
  )

  if (error) return (
    <div className="card p-6 bg-red-50 border-red-200 text-red-700 text-sm">
      {error} <button type="button" onClick={load} className="ml-3 underline text-xs">Retry</button>
    </div>
  )
  ...
  const KPI_ORDER = [
    'bid_win_rate', 'on_time_delivery', 'safety_trir',
    'projected_cash', 'cert_current_pct', 'client_nps',
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-4 flex items-center gap-6 flex-wrap">
        ...
        {[['green','On Target'], ['yellow','Watch'], ['red','Off Track'], ['gray','No Data']].map(([s, label]) => (
          statusCounts[s] > 0 && (
      ...
      {trend.length > 0 && (
        <div className="card p-6">
          <h3 className="font-display font-bold text-brand-navy mb-4">Lead Volume — Rolling 12 Months</h3>
