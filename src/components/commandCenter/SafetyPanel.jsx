  ...
  const handleSaveTalk = async (e) => {
    ...
    try {
      await api.createToolboxTalk({
        ...talkForm,
        crew_count: Number(talkForm.crew_count),
        signed_off: Number(talkForm.signed_off),
      })
  ...
  const handleSaveInc = async (e) => {
    ...
    try {
      await api.createIncident({
        ...incForm,
        osha_recordable: Number(incForm.osha_recordable),
        days_away: Number(incForm.days_away),
      })
    ...
    <div className="space-y-6">
          ...
          {showAdd && (
            <div className="card p-6">
              ...
              <form onSubmit={handleSaveTalk} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ['job_site', 'Job Site', 'text', true],
                  ['topic', 'Topic', 'text', true],
                  ['foreman', 'Foreman', 'text', false],
                ].map(([k, l, type, req]) => (
                  <div key={k}>
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={talkForm.talk_date}
                    onChange={(e) => setT('talk_date', e.target.value)}
                    required
                    className="input text-sm w-full"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    min="0"
                    value={talkForm.crew_count}
                    onChange={(e) => setT('crew_count', e.target.value)}
                    className="input text-sm w-full"
                  />
                </div>
                <div>
                  ...
                  <select
                    value={talkForm.signed_off}
                    onChange={(e) => setT('signed_off', e.target.value)}
                    className="input text-sm w-full"
                  >
                  ...
                  <tbody>
                    {talks.map((t) => (
                      <tr key={t.id} className="border-b border-brand-navy/5">
                        <td className="py-2.5 pr-3 text-brand-navy/60 text-xs">
                        ...
                        <td className="py-2.5 pr-3 text-brand-navy/60 hidden sm:table-cell">
                          {t.foreman || '—'}
                        </td>
                        <td className="py-2.5">
            ...
            <div className="card p-6">
              ...
              <form onSubmit={handleSaveInc} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={incForm.job_site}
                    onChange={(e) => setI('job_site', e.target.value)}
                    required
                    className="input text-sm w-full"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="date"
                    value={incForm.incident_date}
                    onChange={(e) => setI('incident_date', e.target.value)}
                    required
                    className="input text-sm w-full"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setI('incident_type', e.target.value)}
                    className="input text-sm w-full"
                  >
                ...
                <div>
                  ...
                  <select
                    value={incForm.osha_recordable}
                    onChange={(e) => setI('osha_recordable', e.target.value)}
                    className="input text-sm w-full"
                  >
                    <option value="0">No</option>
                    <option value="1">Yes</option>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={incForm.root_cause}
                    onChange={(e) => setI('root_cause', e.target.value)}
                    className="input text-sm w-full"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    min="0"
                    value={incForm.days_away}
                    onChange={(e) => setI('days_away', e.target.value)}
                    className="input text-sm w-full"
                  />
                </div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setI('corrective_action', e.target.value)}
                    rows={2}
                    className="input text-sm w-full"
                  />
                </div>
                  ...
                  >
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <div>
                        ...
                        <span className="ml-3 text-xs text-brand-navy/50">
                      ...
                      >
                        {i.incident_type}
                        {i.osha_recordable ? ' · OSHA' : ''}
                      </span>
                    </div>
                    {i.root_cause && (
                      <p className="text-brand-navy/60 mt-1 text-xs">Root cause: {i.root_cause}</p>
                    )}
                    {i.description && (
                      <p className="text-brand-navy/70 mt-1 text-xs">{i.description}</p>
                    )}
                    {i.corrective_action && (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <StatCard
              ...
              value={oshaRate.trir}
              sub="per 100 workers"
              color={oshaRate.trir <= 3.4 ? 'text-green-600' : 'text-red-600'}
            />
            <StatCard label="Recordables" value={oshaRate.recordable_incidents} />
            <StatCard
              label="Industry Avg"
              value={oshaRate.benchmark_industry_avg}
              sub="BLS construction"
              color="text-brand-navy/60"
            />
            <StatCard
              label="Status"
          ...
          <div className="card p-5 text-sm text-brand-navy/70">
            <p>
              <strong>OSHA TRIR Formula:</strong> (Recordable Incidents × 200,000) / Total Hours
              Worked
            </p>
            <p className="mt-1">
              Based on {oshaRate.total_hours_worked.toLocaleString()} estimated hours. Industry
              average: {oshaRate.benchmark_industry_avg} per 100 workers.
            </p>
          </div>
        </div>
      )}
      ...
      {tab === 'scores' && (
        <div className="card p-6">
          {scores.length === 0 ? (
            <p className="text-center text-brand-navy/30 py-8 text-sm">
              No site data yet. Log talks and incidents to generate scores.
            </p>
          ) : (
            <div className="space-y-4">
              {scores.map((s) => (
                <div key={s.job_site}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-brand-navy/50 text-xs">
                      {s.talks} talks · {s.incidents} incidents · {s.recordables} recordable
                    </span>
                  </div>
